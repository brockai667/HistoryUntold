#!/usr/bin/env python3
"""Doplni banku tem cez GitHub Models (zadarmo). Nika: TECH & AI news (NextByte).
...
EXAMPLE = {
    "title": "Your Phone vs the Moon Landing",
    "place": "",
    "country": "",
    "scenes": [
        {
            "role": "hook",
            "text": "The phone in your pocket is millions of times more powerful than the computer that landed on the Moon.",
            "hook_top": "YOUR PHONE BEATS APOLLO",
            "query": "smartphone hand close up",
            "query2": "person holding phone screen"
        },
        {
            "role": "fact",
            "text": "The Apollo guidance computer ran at about one megahertz, with less memory than a single photo takes today.",
            "query": "apollo spacecraft moon",
            "query2": "retro computer vintage electronics",
            "chips": [
                {
                    "t": "~1 MHZ",
                    "on": "megahertz",
        ...
        {
            "role": "fact",
            "text": "Yet it flew people a million kilometers round trip, because the software was tested like lives depended on it. They did.",
            "query": "rocket launch night",
            "query2": "mission control screens",
            "chips": [
                {
                    "t": "LIVES DEPENDED ON IT",
                    "on": "lives",
                    "style": "accent"
                }
            ]
        ...
        {
            "role": "callout",
            "text": "Meanwhile your phone spends that power on cat videos. Technology grows, priorities differ.",
            "query": "person scrolling phone couch",
            "query2": "social media apps screen",
            "label": "USE THE POWER",
            "sub": "you carry a supercomputer",
            "label_on": "power",
            "punch": "supercomputer"
        },
        {
            "role": "cta",
            "text": "Follow for daily tech and AI news.",
            "query": "futuristic technology city night",
            "query2": "circuit board macro blue"
        }
    ],
    "description": "The Apollo 11 computer ran at ~1 MHz with less memory than one photo - your phone is millions of times faster. Follow for daily tech!",
    "hashtags": [
        "#tech",
        "#technology",
        "#apollo11",
...
CHANNEL_ID = "UC54Qa6hJiAA18ls7qUf-6qA"
...
def build_prompt_fmt(fmt, n, existing_titles, trending=None, perf=""):
    ...
    return (
        f"Generate {n} NEW faceless short-form video topics for a TECH brand (gaming history, how everyday tech works, retro tech, famous hacks and bugs), ALL in the '{fmt}' format.\n"
...
def build_prompt(n, existing_titles, trending=None):
    ...
    return (
        f"Generate {n} NEW faceless short-form video topics for a brand about technology and AI: news, breakthroughs, gadgets and the history of computing, explained simply. "
        ...
        "- tech news must be REAL and widely reported; for history-of-tech use only famous, "
        "well-documented stories. NEVER hype vaporware as fact; label rumors as rumors.\n"
        "- the video must work with sound OFF (hook text + chips tell the story) AND with eyes closed "
        "(voice explains everything).\n"
        "- description: 1-2 engaging sentences, then 'Follow for daily tech!'\n"
...
def valid(t):
    ...
    t.setdefault("description", t["title"] + " Follow for daily tech!")
    t["thumb"] = " ".join(str(t.get("thumb") or "").split()[:4]).upper()
    t.setdefault("hashtags", ["#tech", "#technology", "#apollo11", "#smartphone"])
    return True
